...
harshaEqArea <- readOGR(dsn = paste(rootDir, "harshaDemo", sep=""), # Could use read.shape from spsurvey package
...
#sp2shape(sp.obj=harshaEqArea, # object created with readOGR or read.shape
 #        prjfilename = paste(rootDir, "harshaDemo/harshaEqArea", sep=""), # proj from original shapefile
  #       shpfilename=paste(rootDir, "harshaDemo/harshaEqAreaShp", sep="")) # name of created shapefile.

# EXTRACT ATTRIBUTE TABLE -------------------------
attharsha <- read.dbf(filename = paste(rootDir, "harshaDemo/harshaEqArea", sep=""))
...
harshaSites <- grts(design=harshaDsgn,
                         ...
                         type.frame="area",
                         src.frame="shapefile",
                         in.shape=paste(rootDir, "harshaDemo/harshaEqArea", sep=""),
                         att.frame=attharsha,
                         stratum="Strata",
                         shapefile=TRUE,
                         out.shape=paste(rootDir, "harshaDemo/harshaSites", sep=""),
                         prjfilename=paste(rootDir, "harshaDemo/harshaEqArea", sep=""))
...
writeOGR(obj = harshaEqArea84,  # write projected shapefile to disk for use on field computer
         dsn = paste(rootDir, "harshaDemo", sep=""), 
...
harshaSitesPlot <- readOGR(dsn = paste(rootDir, "harshaDemo", sep=""), 
...
ggmap(harshaSat) +
  ...
  coord_equal() +
  ggtitle("Main Measurement locations for Harsha Lake -- Dry Run 1")

ggsave(filename=paste(rootDir, "harshaDemo/harshaMainSites.tiff", sep=""),
...
ggmap(harshaSat) +
  ...
  coord_equal() +
  ggtitle("Oversample locations for Harsha Lake -- Dry Run 1")

ggsave(filename=paste(rootDir, "harshaDemo/harshaOversampleSites.tiff", sep=""),
       width=8,height=5.5, units="in",
       dpi=800,compression="lzw")
...
ggmap(harshaSat) +
  ...
  coord_equal() +
  ggtitle("All Measurement locations for Harsha Lake -- Dry Run 1")

ggsave(filename=paste(rootDir, "harshaDemo/harshaAllSites.tiff", sep=""),
       width=8,height=5.5, units="in",
       dpi=800,compression="lzw")
