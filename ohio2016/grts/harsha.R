...
##New design for Harsha Lake, adding in unequal probability for convenience
...
harshaEqArea <- readOGR(dsn = paste(rootDir, "harsha", sep=""), # Could use read.shape from spsurvey package
                             layer = "harshaEqArea")  # shapefile name
plot(harshaEqArea) # visualize polygon

# EXTRACT ATTRIBUTE TABLE
attharsha <- read.dbf(filename = paste(rootDir, "harsha/harshaEqArea", sep=""))
...
harshaDsgn <- list("open_water" = list(panel=c(mainSites=10),
                                            seltype="Unequal",
                                            caty.n=c("west" = 6,
                                                     "east" = 4),
                                            over=20),
...
harshaSitesEqArea <- grts(design=harshaDsgn,
                               DesignID="SU", # SU for stratified, unequal
                               type.frame="area",
                               src.frame="shapefile",
                               in.shape=paste(rootDir, "harsha/harshaEqArea", sep=""),
                               att.frame=attharsha,
                               stratum="strata",
                               mdcaty="section",
                               shapefile=TRUE,
                               out.shape=paste(rootDir, "harsha/harshaSitesEqArea", sep=""),
                               prjfilename=paste(rootDir, "harsha/harshaEqArea", sep=""))
...
harshaSitesEqArea <- readOGR(dsn = paste(rootDir, "harsha", sep=""), # Could use read.shape from spsurvey package
                                  layer = "harshaSitesEqArea")  # shapefile name

# add fill-able fields, preparation for analyzing GRTS results
harshaSitesEqArea@data <- mutate(harshaSitesEqArea@data, 
...
writeOGR(obj = harshaSitesEqArea,  # write projected shapefile to disk for use on field computer
         dsn = paste(rootDir, "harsha", sep=""), 
         layer = "harshaSitesEqArea",
         driver = "ESRI Shapefile",
         overwrite_layer = TRUE)


# Print the initial six lines of the survey design
head(harshaSitesEqArea@data)

# Print the survey design summary
summary(harshaSitesEqArea)

plot(harshaEqArea)
points(harshaSitesEqArea$xcoord, harshaSitesEqArea$ycoord)
...
harshaSitesEqAreaPlot <- readOGR(dsn = paste(rootDir, "harsha", sep=""), 
                                      layer = "harshaSitesEqArea")  # shapefile created with grts function
harshaSites84 <- spTransform(x = harshaSitesEqAreaPlot, #reproject
...
write.table(filter(harshaSites84@data, panel == "OverSamp")   %>%
              select(siteID, stratum, mdcaty, long, lat),
            file = paste(rootDir, "harsha/harshaOverSampList.txt", sep=""),
            row.names = FALSE, sep="\t")
...
harshaSites84ListByPanel[[1]]@data        #should be mainSites
harshaSites84ListByPanel[[2]]@data        #should be OverSamp sites

# Write 'mainSites' to disk
writeOGR(obj = harshaSites84ListByPanel[[1]], # pulls out 'mainSites' shapefile from list.  Should confirm.
         dsn = paste(rootDir, "harsha", sep=""), 
         layer = "harshaSites84mainSites",
         driver = "ESRI Shapefile",
         overwrite_layer = TRUE)
# Write 'OverSamp' to disk
writeOGR(obj = harshaSites84ListByPanel[[2]], # pulls out 'OverSamp' shapefile from list.  Should confirm.
         dsn = paste(rootDir, "harsha", sep=""), 
         layer = "harshaSites84OverSamp",
         driver = "ESRI Shapefile",
         overwrite_layer = TRUE)
...
ggmap(harshaSat) +
  ylab("Latitude") +
  xlab ("Longitude") +
  geom_polygon(data=harshaEqArea84.f, aes(long, lat, group=group, fill=section)) +
  scale_fill_manual(values = c("#000066", "#333399", "#006666")) + # colors from http://www.cookbook-r.com/Graphs/Colors_(ggplot2)/
  geom_point(data=filter(harshaSites84@data, panel == "mainSites"), #only main sites
             aes(x=long, y=lat),
             size = 2, color = "#F8766D") + # specify color to be consistent across maps
  geom_text(data=filter(harshaSites84@data, panel == "mainSites"), #only main sites
            aes(label=siteID, x=long, y=lat),
            hjust=1.2, vjust=0, size=2, color = "#F8766D") +     ### "#00BFC4" is blue
  coord_equal() +                                                ### "#F8766D" is pink
  ggtitle("Main Measurement locations for Lake Harsha")

ggsave(filename=paste(rootDir, "harsha/harshaMainSites.tiff", sep=""),
...
ggmap(harshaSat) +
  ...
  coord_equal() +
  ggtitle("Oversample locations for Lake Harsha")

ggsave(filename=paste(rootDir, "harsha/harshaOversampleSites.tiff", sep=""),
       width=8,height=5.5, units="in",
       dpi=800,compression="lzw")
...
ggmap(harshaSat) +
  ...
  coord_equal() +
  ggtitle("All Measurement locations for Lake Harsha")

ggsave(filename=paste(rootDir, "harsha/harshaAllSites.tiff", sep=""),
       width=8,height=5.5, units="in",
       dpi=800,compression="lzw")
