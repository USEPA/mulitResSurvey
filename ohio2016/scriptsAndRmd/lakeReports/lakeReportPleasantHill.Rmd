---
title: "Water Quality and Methane Emissions at Pleasant Hill Lake, Ohio"
...
This preliminary report presents results from the USEPA 2016 measurement campaign relevant to Pleasant Hill Lake which is located in north-central Ohio and managed by the Muskingum Watershed Conservancy District. We sampled Pleasant Hill Lake on July 19^th^ - 20^th^ of 2016. These data will be included in a formal peer-reviewed publication to be submitted for publication in early 2018.  This preliminary report includes:  
...
3. A 3D map of the reservoir showing the measurement results for :
    + CH~4~ emissions
    + Chlorophyll a
4. Figures showing how Pleasant Hill Lake compared to the other 31 reservoirs in the study in terms of:
    + CH~4~ emissions
    + Total phosphorus 
    + Total nitrogen
    + Chlorophyll a
5. Tables summarizing the other measured water quality values at each site within Pleasant Hill Lake   
 
Thank you for your help in including Pleasant Hill Lake in this project. 
...
Pleasant Hill Lake, located in north-central Ohio, is one of 16 reservoirs in the Muskingum Watershed Conservancy District. We sampled Pleasant Hill Lake on July 19^th^ - 20^th^ of 2016. The sampling sites were chosen using a generalized random tessellation stratified design ("GRTS"), an approach which combines elements of systematic and random survey designs which allows for the random allocation of sampling sites with maximum  spatial coverage of the reservoir. We used a GPS and geographic information system (GIS) software to locate each sampling site (+/- 30 meters).  Chlorophyll a, dissolved oxygen, pH, specific conductivity, water temperature, turbidity, and CH~4~ emissions were measured at all sampling sites. Nutrient concentrations were measured at one deep site near the dam and one shallow site near the inflow. 
  
  

```{r results='hide', message=FALSE, warning=FALSE, echo=FALSE, fig.width=7, fig.height=6.0, fig.cap='*Location of the fifteen sample sites within Pleasant Hill Lake, located in north-central Ohio. Water samples for nutrient analysis were collected from one deep and one shallow site. Satellite image from Google Maps.*'}


# Read and project spatial points dataframe for plotting
pleasantHillSitesPlot <- readOGR(dsn = paste(globalWd, "pleasantHill", sep=""), 
                          layer = "pleasantHillSitesEqAreaData")  # shapefile with data

pleasantHillSites84 <- spTransform(x = pleasantHillSitesPlot, #reproject
...
pleasantHillSites84@data <- mutate(pleasantHillSites84@data,
                              chem = ifelse(siteID %in% c("S-07", "S-34"), 
                                  "chem",
                                  "no_chem"))
...
ggmap(pleasantHillSat) +
  ylab("Latitude") +
  xlab ("Longitude") +
  geom_point(data=filter(pleasantHillSites84@data, EvalStatus == "sampled"), #only main sites
             aes(x=long, y=lat, color = chem),
             size = 2) + 
  theme(plot.title = element_text(hjust = 0.5)) +  #plot title justify center
  geom_text_repel(data=filter(pleasantHillSites84@data, EvalStatus == "sampled"), #only main sites
  ...
  coord_equal() +
  ggtitle("Pleasant Hill Lake Sample Sites")
```
...
Previous studies have found a general pattern of higher CH~4~ emissions in the river-reservoir transition (tributary) area, and lower emissions in the deeper downstream waters. There were no strong spatial patterns in CH~4~ emission rates in north-central Ohio's Pleasant Hill Lake.

```{r echo=FALSE, warning=F, message=F, error=F, fig.width=7, fig.height=7, fig.cap='*Methane emissions measured at the 15 sample sites within Pleasant Hill Lake from July 19^th^ to 20^th^, 2016. The height of each "lollipop" corresponds to the emission rate shown on the vertical z-axis in units of milligrams CH~4~ per square meter of lake surface per hour.*' }

# Get polygon
pleasantHillEqArea84 <- readOGR(dsn = paste(globalWd, "pleasantHill", sep=""), 
                           layer = "pleasantHillEqArea84",  # shapefile name
                            verbose = FALSE)  #keep from reporting progress

# Dissolve strata from polygon
pleasantHillEqArea84 <- gUnaryUnion(pleasantHillEqArea84, id = pleasantHillEqArea84@data$GNIS_Name)

# Add emission to point shp
pleasantHillSites84@data <- merge(pleasantHillSites84@data, 
                             filter(eqAreaData, Lake_Name == "Pleasant Hill Lake") %>%
...
coords = fortify(pleasantHillEqArea84)
...
with(pleasantHillSites84@data, {
  ch4_plot <- scatterplot3d_edit(x=coords$long, y=coords$lat, z=rep(0,length(coords$long)),   # x y and z data, respectively
                                   ...
                                   type="l",                                      # plot type "l" = line, "h" = lollipop with line connecting to horizontal plane 
  ...
  y.d <- dims[3] + 0.14*diff(dims[3:4])        # define y position of label
  text(x.d, y.d, "Inflow")             # add label.  srt sets angle
  
  # add dam label
  dims <- par("usr")                        # format of 'dims' is vector of: [xmin,xmax,ymin,ymax]
  x.i <- dims[1] + 0.59*diff(dims[1:2])       # define x position of label
...
The lowest Chlorophyll a concentrations occurred immediately above the dam.  Chlorophyll concentrations were relatively uniform across the rest of the reservoir (Figure 3).  

```{r echo=FALSE, warning=F, message=F, error=F, fig.width=7, fig.height=7, fig.cap='*Chlorophyll a concentrations measured at the 15 sample sites within Pleasant Hill Lake on July 19^th^, 2016. The height of each "lollipop" corresponds to the concentration shown on the vertical z-axis in units of micrograms per liter of water.*' }
##########################################################################
### chlorophyll plot
with(pleasantHillSites84@data, {
...
# Highlight Pleasant Hill Lake (lake of interest) data with color
plotColor <- ifelse(meanVariance.c.lake.lu$Lake_Name == "Pleasant Hill Lake",
                    "red", "black")
...
ggplot(meanVariance.c.lake.lu,
  ...
  theme(axis.title.y = element_blank(), # Eliminate x-axis title
        ...
        axis.text.y = element_text(color = c(rep("black", 22), "red", rep("black", 9)),
...
ggplot(meanVariance.c.lake.lu,
  ...
  theme(axis.title.y = element_blank(), # Eliminate x-axis title
        plot.title = element_text(hjust = 0.5),  #plot title justify center
        axis.text.y = element_text(color = c(rep("black", 20), "red", rep("black", 11)),
        face = c(rep("plain", 20), "bold", rep("plain", 11)))
        ) +
...
```{r echo=FALSE, warning=FALSE, fig.width=7, fig.height=7, fig.cap='*TP concentrations at a deep and shallow site in each reservoir included in the study.  Pleasant Hill Lake is highlighted in red.  See Fig. 1 for sample site locations.*'  }
...
# Highlight Pleasant Hill Lake (lake of interest) data with color
plotColor <- ifelse(tp.tn$fLake_Name == "Pleasant Hill Lake",
                    "red", "black")
...
ggplot(tp.tn,
  ...
  theme(axis.text.y = element_text(color = c(rep("black", 23), "red", rep("black", 8)),
        face = c(rep("plain", 23), "bold", rep("plain", 8)))) +
...
```{r echo=FALSE, warning=FALSE, fig.width=7, fig.height=7, fig.cap='*TN concentrations at a deep and shallow site in each reservoir included in the study.  Pleasant Hill Lake is highlighted in red.  See Fig. 1 for sample site locations.*'  }
...
Table 1: Surface (0.1 m) water quality parameters measured via sonde at each site on July 19^th^, 2016. "Site Depth" is the measure of how deep the lake was at each site. 
...
Table 2: Surface (0.1 m) water chemistry parameters measured at one shallow and one deep site on on July 19^th^, 2016. "Site Depth" is the measure of how deep the lake was at each site. 
```{r warning=FALSE, echo=FALSE} 
pleasantHillTableChem <- filter(eqAreaData, Lake_Name == "Pleasant Hill Lake",
                      EvalStatus == "sampled")  %>%
                  filter(!is.na(TN)) %>%
                  select(siteID, LatSamp, LongSmp, wtrDpth,  #site identification  
...
pleasantHillTableChemOrder <- plyr::arrange(pleasantHillTableChem, wtrDpth)
   
                         
kable(pleasantHillTableChemOrder,
      align = "c",
