---
title: "Water Quality and Methane Emissions at Charles Mill Lake, Ohio"
...
This preliminary report presents results from the USEPA July 18^th^ - 19^th^ 2016 measurement campaign at Charles Mill Lake which is located in north-central Ohio and managed by the Muskingum Watershed Conservancy District. These data will be included in a formal peer-reviewed publication to be submitted for publication in early 2018.  This preliminary report includes:  
...
3. A 3D map of the reservoir showing the measurement results for :
    + CH~4~ emissions
    + Chlorophyll a
4. Figures showing how Charles Mill Lake compared to the other 31 reservoirs in the study in terms of:
    + CH~4~ emissions
    + Total phosphorus 
    + Total nitrogen
    + Chlorophyll a
5. Tables summarizing the other measured water quality values at each site within Charles Mill Lake   
 
Thank you for your help in including Charles Mill Lake in this project. 
...
Charles Mill Lake, located in north-central Ohio, is one of 16 reservoirs in the Muskingum Watershed Conservancy District. We sampled Charles Mill Lake on July 18^th^ - 19^th^ of 2016. The sampling sites were chosen using a generalized random tessellation stratified design ("GRTS"), an approach which combines elements of systematic and random survey designs which allows for the random allocation of sampling sites with maximum  spatial coverage of the reservoir. We used a GPS and geographic information system (GIS) software to locate each sampling site (+/- 30 meters).  Chlorophyll a, dissolved oxygen, pH, specific conductivity, water temperature, turbidity, and CH~4~ emissions were measured at all sampling sites.  Nutrient concentrations were measured at one deep site near the dam and one shallow site near the inflow. 

```{r results='hide', message=FALSE, warning=FALSE, echo=FALSE, fig.width=7, fig.height=6.0, fig.cap='*Location of the fifteen sample sites within Charles Mill Lake, located in north-central Ohio. Water samples for nutrient analysis were collected from one deep and one shallow site. Satellite image from Google Maps.*'}


# Read and project spatial points dataframe for plotting
charlesMillSitesPlot <- readOGR(dsn = paste(globalWd, "charlesMill", sep=""), 
                          layer = "charlesMillSitesEqAreaData")  # shapefile with data

charlesMillSites84 <- spTransform(x = charlesMillSitesPlot, #reproject
...
charlesMillSites84@data <- mutate(charlesMillSites84@data,
                              chem = ifelse(siteID %in% c("US-01", "US-16"), 
                                  "chem",
                                  "no_chem"))
...
charlesMillSat <- get_map(location = bbox,
                    color = "color",
                    source = "google",
                    maptype = "satellite")
...
ggmap(charlesMillSat) +
  ylab("Latitude") +
  xlab ("Longitude") +
  geom_point(data=filter(charlesMillSites84@data, EvalStatus == "sampled"), #only main sites
  ...
  geom_text_repel(data=filter(charlesMillSites84@data, EvalStatus == "sampled"), #only main sites
 ...
 scale_color_manual(values = c("yellow", "white"), #alternate color: "#F8766D" "#00BFC4"
  ...
  guides(color = guide_legend(title = NULL)) +
   coord_equal() +
  ggtitle("Charles Mill Lake Sample Sites")
```
...
All sampling locations in Charles Mill Lake, located in north-central Ohio, were emitting CH~4~ to the atmosphere. However, there is not a clear within-lake spatial pattern (Fig. 2).

```{r echo=FALSE, warning=F, message=F, error=F, fig.width=7, fig.height=7, fig.cap='*Methane emissions measured at the 15 sample sites within Charles Mill Lake from July 18^th^ to 19^th^, 2016. The height of each "lollipop" corresponds to the emission rate shown on the vertical z-axis in units of milligrams CH~4~ per square meter of lake surface per hour.*' }

# Get polygon
charlesMillEqArea84 <- readOGR(dsn = paste(globalWd, "charlesMill", sep=""), 
                           layer = "charlesMillEqArea84",  # shapefile name
                            verbose = FALSE)  #keep from reporting progress

# Dissolve strata from polygon
charlesMillEqArea84 <- gUnaryUnion(charlesMillEqArea84, id = charlesMillEqArea84@data$Lake_Name)

# Add emission to point shp
charlesMillSites84@data <- merge(charlesMillSites84@data, 
                             filter(eqAreaData, Lake_Name == "Charles Mill Lake") %>%
...
coords = fortify(charlesMillEqArea84)
...
Chlorophyll a was abundant throughout the lake (Fig. 3).  Overall, Charles Mill Lake, located in north-central Ohio, had the second highest chloropyll a abundance of the 32 lakes (Fig. 5). 

```{r echo=FALSE, warning=F, message=F, error=F, fig.width=7, fig.height=7, fig.cap='*Chlorophyll a concentrations measured at the 15 sample sites within Charles Mill Lake on July 18^th^, 2016. The height of each "lollipop" corresponds to the concentration shown on the vertical z-axis in units of micrograms per liter of water.*' }
##########################################################################
### chlorophyll plot
with(charlesMillSites84@data, {
  chl_plot <- scatterplot3d_edit(x=coords$long, y=coords$lat, z=rep(0,length(coords$long)),   # x y and z data, respectively
                                   ...
                                   zlim=range(0, 80),                            # range of z data shown
                                   labs.line.z=-0.5,                              # adjustment of the z axis tick labels in margin lines
...
plotColor <- ifelse(meanVariance.c.lake.lu$Lake_Name == "Charles Mill Lake",
                    "red", "black")
...
ggplot(meanVariance.c.lake.lu,
  ...
  theme(axis.title.y = element_blank(), # Eliminate x-axis title
    ...
    theme(axis.text.y = element_text(color = c(rep("black", 14), "red", rep("black", 17)),
        face = c(rep("plain", 14), "bold", rep("plain", 17)))) +
...
ggplot(meanVariance.c.lake.lu,
  ...
  ggtitle("Mean Chlorophyll a (95% CI)") +
      theme(axis.text.y = element_text(color = c(rep("black", 30), "red", rep("black", 1)),
        face = c(rep("plain", 30), "bold", rep("plain", 1))))
```

\newpage

```{r echo=FALSE, warning=FALSE, fig.width=7, fig.height=7, fig.cap='*TP concentrations at a deep and shallow site in each reservoir included in the study.  Charles Mill Lake is highlighted in red.  See Fig. 1 for sample site locations.*'  }
...
# Highlight Charles Mill Lake (lake of interest) data with color
plotColor <- ifelse(tp.tn$fLake_Name == "Charles Mill Lake",
                    "red", "black")
...
```{r echo=FALSE, warning=FALSE, fig.width=7, fig.height=7, fig.cap='*TN concentrations at a deep and shallow site in each reservoir included in the study.  Charles Mill Lake is highlighted in red.  See Fig. 1 for sample site locations.*'  }
...
Table 1: Surface (0.1 m) water quality parameters measured via sonde at each site on July 18^th^, 2016. "Site Depth" is the measure of how deep the lake was at each site. 
...
Table 2: Surface (0.1 m) water chemistry parameters measured at one shallow and one deep site on July 18^th^, 2016. "Site Depth" is the measure of how deep the lake was at each site. 
```{r warning=FALSE, echo=FALSE} 
charlesMillTableChem <- filter(eqAreaData, Lake_Name == "Charles Mill Lake",
                      EvalStatus == "sampled")  %>%
                  filter(!is.na(TN)) %>%
                  select(siteID, LatSamp, LongSmp, wtrDpth,  #site identification  
...
charlesMillTableChemOrder <- plyr::arrange(charlesMillTableChem, wtrDpth)
   
                         
kable(charlesMillTableChemOrder,
      align = "c",
