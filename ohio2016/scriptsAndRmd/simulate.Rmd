...
The factorial model is trying to fit too many parameters for the sample size of 32, and isn't going to be an ideal choice.
...
What we really want is to see how the fitted surfaces vary over many simulated datasets and fits.  This is a place where more discussion is worthwhile.  The Task Description suggested a power analysis for effect sizes was required, but further communication has indicated the big question is "Is 30 reservoirs enough?".  We should talk about the sample size question in more detail... enough for what?  I'm guessing that, ideally, you want to see parameter values in the model that are significantly different from 0.  The preliminary results suggest it's going to be difficult to nail down the interaction term, so I focused on main effects in the simulations.
