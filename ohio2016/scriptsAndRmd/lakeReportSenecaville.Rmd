---
title: "Water Quality and Methane Emissions at Seneca Lake"
...
This preliminary report presents results from the USEPA 2016 measurement campaign relevant to Seneca Lake. These data will be included in a formal peer-reviewed publication to be submitted for publication in early 2018.  This preliminary report includes:  
...
ggmap(senecavilleSat) +
  ...
  geom_text(data=filter(senecavilleSites84@data, EvalStatus == "sampled"), #only main sites
...
Previous studies have found a general pattern of higher CH~4~ emissions in the river-reservoir transition (tributary) area, and lower emissions in the deeper downstream waters. This pattern was not observed at Seneca Lake. In fact, Seneca Lake went against this trend, with the highest CH~4~ emissions observed in deeper waters near the dam, gradually decreasing to the lowest emissions observed in the shallower southeast area.

```{r echo=FALSE, warning=F, message=F, error=F, fig.width=7, fig.height=7, fig.cap='*Methane emissions measured at the 15 sample sites within Seneca Lake. The height of each "lollipop" corresponds to the emission rate shown on the vertical z-axis in units of milligrams CH~4~ per square meter of lake surface per hour.*' }

# Get polygon
senecavilleEqArea <- readOGR(dsn = paste(globalWd, "senecaville", sep=""), 
                           layer = "senecavilleEqArea",  # shapefile name
                            verbose = FALSE)  #keep from reporting progress

# Dissolve strata from polygon
senecavilleEqArea <- gUnaryUnion(senecavilleEqArea, id = senecavilleEqArea@data$Lake_Name)

# Add emission to point shp
senecavilleSitesPlot@data <- merge(senecavilleSitesPlot@data, 
...
coords = fortify(senecavilleEqArea)
...
with(senecavilleSitesPlot@data, {
...
```{r echo=FALSE, warning=F, message=F, error=F, fig.width=7, fig.height=7, fig.cap='*Chlorophyll a concentrations measured at the 15 sample sites within Seneca Lake. The height of each "lollipop" corresponds to the concentration shown on the vertical z-axis in units of micrograms per liter of water.*' }
##########################################################################
### chlorophyll plot
with(senecavilleSitesPlot@data, {
