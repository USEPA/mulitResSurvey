---
title: "Water Quality and Methane Emissions at Seneca Lake, Ohio"
...
This preliminary report presents results from the USEPA 2016 measurement campaign relevant to Seneca Lake (aka Senecaville Lake), a reservoir located in east-central Ohio and managed by the Muskingum Watershed Conservancy District. These data will be included in a formal peer-reviewed publication to be submitted for publication in early 2018.  This preliminary report includes:  
...
3. A 3D map of the reservoir showing the measurement results for :
    + CH~4~ emissions
    + Chlorophyll a
4. Figures showing how Seneca Lake compared to the other 31 reservoirs in the study in terms of:
    + CH~4~ emissions
    + Total phosphorus 
    + Total nitrogen
    + Chlorophyll a
5. Tables summarizing the other measured water quality values at each site within Seneca Lake   
 
Thank you for your help in including Seneca Lake in this project. 
...
senecavilleSites84@data <- mutate(senecavilleSites84@data,
                              chem = ifelse(siteID %in% c("S-21", "S-04"), 
                                  "chem",
                                  "no_chem"))
...
ggmap(senecavilleSat) +
  ylab("Latitude") +
  xlab ("Longitude") +
  geom_point(data=filter(senecavilleSites84@data, EvalStatus == "sampled"), #only main sites
  ...
  geom_text_repel(data=filter(senecavilleSites84@data, EvalStatus == "sampled"), #only main sites
 ...
 scale_color_manual(values = c("yellow", "white"), #alternate color: "#F8766D" "#00BFC4"
  ...
  guides(color = guide_legend(title = NULL)) +
   coord_equal() +
  ggtitle("Seneca Lake Sample Sites")
```
...
Previous studies have found a general pattern of higher CH~4~ emissions in the river-reservoir transition (tributary) area, and lower emissions in the deeper downstream waters. This pattern was not observed at Seneca Lake. In fact, the measurement results at Seneca Lake went against this trend, with the highest CH~4~ emissions observed in deeper waters near the dam, gradually decreasing to the lowest emissions observed in the shallower southeast area near the inflow.

```{r echo=FALSE, warning=F, message=F, error=F, fig.width=7, fig.height=7, fig.cap='*Methane emissions measured at the 15 sample sites within Seneca Lake, Ohio. The height of each "lollipop" corresponds to the emission rate shown on the vertical z-axis in units of milligrams CH~4~ per square meter of lake surface per hour.*' }

# Get polygon
senecavilleEqArea84 <- readOGR(dsn = paste(globalWd, "senecaville", sep=""), 
                           layer = "senecavilleEqArea84",  # shapefile name
                            verbose = FALSE)  #keep from reporting progress

# Dissolve strata from polygon
senecavilleEqArea84 <- gUnaryUnion(senecavilleEqArea84, id = senecavilleEqArea84@data$Lake_Name)

# Add emission to point shp
senecavilleSites84@data <- merge(senecavilleSites84@data, 
                             filter(eqAreaData, Lake_Name == "Senecaville Lake") %>%
...
coords = fortify(senecavilleEqArea84)
...
with(senecavilleSites84@data, {
  ...
  x <- dims[2] - 0.08*diff(dims[1:2])       # define x position of label
  y <- dims[3] + 0.1*diff(dims[3:4])       # define y position of label
  text(x,y,"Longitude", srt=45)             # add label.  srt sets angle
  
  # add dam label
  dims <- par("usr")                        # format of 'dims' is vector of: [xmin,xmax,ymin,ymax]
  x.d <- dims[1] + 0.15*diff(dims[1:2])       # define x position of label
  y.d <- dims[3] + 0.17*diff(dims[3:4])        # define y position of label
  text(x.d, y.d, "Dam")             # add label.  srt sets angle
  
  # add inflow label
  dims <- par("usr")                        # format of 'dims' is vector of: [xmin,xmax,ymin,ymax]
  x.i <- dims[1] + 0.68*diff(dims[1:2])       # define x position of label
  y.i <- dims[3] + 0.19*diff(dims[3:4])        # define y position of label
...
The chlorophyll a levels did not reflect the pattern seen in the CH~4~ emissions. Chlorophyll a was relatively constant across Seneca Lake. 

```{r echo=FALSE, warning=F, message=F, error=F, fig.width=7, fig.height=7, fig.cap='*Chlorophyll a concentrations measured at the 15 sample sites within Seneca Lake, Ohio. The height of each "lollipop" corresponds to the concentration shown on the vertical z-axis in units of micrograms per liter of water.*' }
##########################################################################
### chlorophyll plot
with(senecavilleSites84@data, {
  chl_plot <- scatterplot3d_edit(x=coords$long, y=coords$lat, z=rep(0,length(coords$long)),   # x y and z data, respectively
                                   ...
                                   zlim=range(0, 30),                            # range of z data shown
                                   labs.line.z=-0.5,                              # adjustment of the z axis tick labels in margin lines
  ...
  # add dam label
  dims <- par("usr")                        # format of 'dims' is vector of: [xmin,xmax,ymin,ymax]
  x.d <- dims[1] + 0.13*diff(dims[1:2])       # define x position of label
  y.d <- dims[3] + 0.17*diff(dims[3:4])        # define y position of label
  text(x.d, y.d, "Dam")             # add label.  srt sets angle
  
  # add inflow label
  dims <- par("usr")                        # format of 'dims' is vector of: [xmin,xmax,ymin,ymax]
  x.i <- dims[1] + 0.72*diff(dims[1:2])       # define x position of label
  y.i <- dims[3] + 0.17*diff(dims[3:4])        # define y position of label
  text(x.i, y.i, "Inflow")                       # add label. 
...
# Highlight Seneca Lake (lake of interest) data with color
plotColor <- ifelse(meanVariance.c.lake.lu$Lake_Name == "Senecaville Lake",
                    "red", "black")
...
ggplot(meanVariance.c.lake.lu,
  ...
  theme(axis.title.y = element_blank(), # Eliminate x-axis title
    ...
    theme(axis.text.y = element_text(color = c(rep("black", 11), "red", rep("black", 20)),
        face = c(rep("plain", 11), "bold", rep("plain", 20)))) +
...
ggplot(meanVariance.c.lake.lu,
  ...
  ggtitle("Mean Chlorophyll a (95% CI)") +
      theme(axis.text.y = element_text(color = c(rep("black", 22), "red", rep("black", 9)),
        face = c(rep("plain", 22), "bold", rep("plain", 9))))
```

\newpage

```{r echo=FALSE, warning=FALSE, fig.width=7, fig.height=7, fig.cap='*TP concentrations at a deep and shallow site in each reservoir included in the study.  Seneca Lake is highlighted in red.  See Fig. 1 for sample site locations.*'  }
...
# Highlight Seneca Lake (lake of interest) data with color
plotColor <- ifelse(tp.tn$fLake_Name == "Senecaville Lake",
                    "red", "black")
...
ggplot(tp.tn,
  ...
  theme(axis.text.y = element_text(color = c(rep("black", 21), "red", rep("black", 10)),
        face = c(rep("plain", 21), "bold", rep("plain", 10)))) +
...
```{r echo=FALSE, warning=FALSE, fig.width=7, fig.height=7, fig.cap='*TN concentrations at a deep and shallow site in each reservoir included in the study.  Seneca Lake is highlighted in red.  See Fig. 1 for sample site locations.*'  }
...
ggplot(tp.tn,
  ...
  theme(axis.text.y = element_text(color = c(rep("black", 16), "red", rep("black", 15)),
        face = c(rep("plain", 16), "bold", rep("plain", 15)))) +
  scale_color_manual(values = c("black", "red")) +
  guides(color=FALSE) +
  theme(legend.title = element_blank())
...
```{r warning=FALSE, echo=FALSE} 
senecavilleTableChem <- filter(eqAreaData, Lake_Name == "Senecaville Lake",
                      EvalStatus == "sampled")  %>%
                  filter(!is.na(TN)) %>%
                  select(siteID, LatSamp, LongSmp, wtrDpth,  #site identification  
...
senecavilleTableChemOrder <- plyr::arrange(senecavilleTableChem, wtrDpth)
   
                         
kable(senecavilleTableChemOrder,
      align = "c",
