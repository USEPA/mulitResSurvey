---
title: "Water Quality and Methane Emissions at Cave Run Lake, Kentucky"
...
3. A 3D map of the reservoir showing the measurement results for :
    + CH~4~ emissions
    + Chlorophyll a
4. Figures showing how Cave Run Lake compared to the other 31 reservoirs in the study in terms of:
    + CH~4~ emissions
    + Total phosphorus 
    + Total nitrogen
    + Chlorophyll a
5. Tables summarizing the other measured water quality values at each site within Cave Run Lake   
 
Thank you for your help in including Cave Run Lake in this project. 
...
caveRunSites84@data <- mutate(caveRunSites84@data,
                              chem = ifelse(siteID %in% c("SU-46", "SU-07"), 
                                  "chem",
                                  "no_chem"))
...
ggmap(caveRunSat) +
  ylab("Latitude") +
  xlab ("Longitude") +
  geom_point(data=filter(caveRunSites84@data, EvalStatus == "sampled"), #only main sites
             aes(x=long, y=lat, color = chem),
             size = 0.5) + # specify color to be consistent across maps
  theme(plot.title = element_text(hjust = 0.5)) +  #plot title justify center
  geom_text_repel(data=filter(caveRunSites84@data, EvalStatus == "sampled"), #only main sites
  ...
  guides(color = guide_legend(title = NULL)) +
   coord_equal() +
  ggtitle("Cave Run Lake Sample Sites")
```
...
Previous studies have found a general pattern of higher CH~4~ emissions in the river-reservoir transition (tributary) area, and lower emissions in the deeper downstream waters. Cave Run Lake displayed this pattern: the highest emissions were observed in the upper half of the reservoir nearest the inflow.

```{r echo=FALSE, warning=F, message=F, error=F, fig.width=7, fig.height=7, fig.cap='*Methane emissions measured at the 30 sample sites within Cave Run Lake, located in eastern Kentucky. The height of each "lollipop" corresponds to the emission rate shown on the vertical z-axis in units of milligrams CH~4~ per square meter of lake surface per hour.*' }


caveRunEqArea84 <- readOGR(dsn = paste(globalWd, "caveRun", sep=""), # get polygon
                           layer = "caveRunEqArea84",  # shapefile name
                            verbose = FALSE)  #keep from reporting progress

# Dissolve strata from polygon
caveRunEqArea84 <- gUnaryUnion(caveRunEqArea84, id = caveRunEqArea84@data$GNIS_Name)

caveRunSites84@data <- merge(caveRunSites84@data, # add emission to point shp
                             filter(eqAreaData, Lake_Name == "Cave Run Lake") %>%
...
coords = fortify(caveRunEqArea84)
...
with(caveRunSites84@data, {
  ch4_plot <- scatterplot3d_edit(x=coords$long, y=coords$lat, z=rep(0,length(coords$long)),   # x y and z data, respectively
                                   ...
                                   ylab="",                                       # y-axis label (left blank here to reposition later)
                                   y.margin.add = 0.5,                            #add space between tick mark and axis labels
                                   cex.axis = 0.7, 
                                   cex.lab = 1.1,
                                   zlab=expression(CH[4]~emission~rate~(mg~ CH[4]~ m^{-2}~ hr^{-1})),    # z-axis label
                                   zlim=range(0,50),                              # range of z data shown
  ...
  text(x,y,"Longitude", srt=83)             # add label.  srt sets angle
  
    # add dam label
  dims <- par("usr")                        # format of 'dims' is vector of: [xmin,xmax,ymin,ymax]
  x.d <- dims[1] + 0.17*diff(dims[1:2])       # define x position of label
  y.d <- dims[3] + 0.32*diff(dims[3:4])        # define y position of label
  text(x.d, y.d, "Dam")             # add label.  srt sets angle
  
  # add inflow label
  dims <- par("usr")                        # format of 'dims' is vector of: [xmin,xmax,ymin,ymax]
  x.i <- dims[1] + 0.64*diff(dims[1:2])       # define x position of label
  y.i <- dims[3] + 0.06*diff(dims[3:4])        # define y position of label
...
```{r echo=FALSE, warning=F, message=F, error=F, fig.width=7, fig.height=7, fig.cap='*Chlorophyll a concentrations measured at the 30 sample sites within Cave Run Lake, located in eastern Kentucky. The height of each "lollipop" corresponds to the concentration shown on the vertical z-axis in units of micrograms per liter of water.*' }
##########################################################################
### chlorophyll plot
with(caveRunSites84@data, {
  ...
  dims <- par("usr")                        # format of 'dims' is vector of: [xmin,xmax,ymin,ymax]
  x <- dims[2] - 0.03*diff(dims[1:2])       # define x position of label
  y <- dims[3] + 0.25*diff(dims[3:4])        # define y position of label
  text(x,y,"Longitude", srt=85)             # add label.  srt sets angle
  
  # add dam label
  dims <- par("usr")                        # format of 'dims' is vector of: [xmin,xmax,ymin,ymax]
  x.d <- dims[1] + 0.17*diff(dims[1:2])       # define x position of label
  y.d <- dims[3] + 0.41*diff(dims[3:4])        # define y position of label
  text(x.d, y.d, "Dam")             # add label.  srt sets angle
  
  # add inflow label
  dims <- par("usr")                        # format of 'dims' is vector of: [xmin,xmax,ymin,ymax]
  x.i <- dims[1] + 0.65*diff(dims[1:2])       # define x position of label
  y.i <- dims[3] + 0.1*diff(dims[3:4])        # define y position of label
  text(x.i, y.i, "Inflow")                       # add label. 
...
plotColor <- ifelse(meanVariance.c.lake.lu$fLake_Name == "Cave Run Lake",
                    "red", "black")
...
```{r echo=FALSE, warning=FALSE, fig.width=7, fig.height=7, fig.cap='*TP concentrations at a deep and shallow site in each reservoir included in the study.  Cave Run Lake is highlighted in red.  See Fig. 1 for sample site locations.*'  }
...
# Highlight Cave Run Lake (lake of interest) data with color
plotColor <- ifelse(tp.tn$fLake_Name == "Cave Run Lake",
                    "red", "black")
...
ggplot(tp.tn,
  ...
  ggtitle("Total Phosphorus") +
    theme(axis.text.y = element_text(color = c(rep("red", 1), rep("black", 31)),
        face = c(rep("bold", 1), rep("plain", 31)))) +
...
```{r echo=FALSE, warning=FALSE, fig.width=7, fig.height=7, fig.cap='*TN concentrations at a deep and shallow site in each reservoir included in the study.  Cave Run Lake is highlighted in red.  See Fig. 1 for sample site locations.*'  }
...
ggplot(tp.tn,
  ...
  ggtitle("Total Nitrogen") +
    theme(axis.text.y = element_text(color = c(rep("black", 2), "red", rep("black", 29)),
        face = c(rep("plain", 2), "bold", rep("plain", 29)))) +
  scale_color_manual(values = c("black", "red")) +
  guides(color=FALSE) +
  theme(legend.title = element_blank())
...
caveRunTableSonde <- filter(eqAreaData, Lake_Name == "Cave Run Lake", 
                      EvalStatus == "sampled",
                      siteID != "SU-04") %>%  # no sonde data from this site
                select(siteID, LatSamp, LongSmp, wtrDpth,  #site identification  
                chla_S, DO__L_S,  pH_S, SpCn__S, Tmp_C_S,  TrNTU_S) #shallow sonde

kable(caveRunTableSonde,
      align = "c",
...
```{r warning=FALSE, echo=FALSE} 
caveRunTableChem <- filter(eqAreaData, Lake_Name == "Cave Run Lake",
                      EvalStatus == "sampled")  %>%
                  filter(!is.na(TN)) %>%
                  select(siteID, LatSamp, LongSmp, wtrDpth,  #site identification  
...
caveRunTableChemOrder <- plyr::arrange(caveRunTableChem, wtrDpth)
   
                         
kable(caveRunTableChemOrder,
      align = "c",
