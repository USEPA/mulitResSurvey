---
title: "Water Quality and Methane Emissions at Cave Run Lake"
...
This preliminary report presents results from the USEPA 2016 measurement campaign relevant to Cave Run Lake. These data will be included in a formal peer-reviewed publication to be submitted for publication in early 2018.  This preliminary report includes:  
...
ggmap(caveRunSat) +
  ...
  geom_text(data=filter(caveRunSites84@data, EvalStatus == "sampled"), #only main sites
...
```{r echo=FALSE, warning=F, message=F, error=F, fig.width=7, fig.height=7, fig.cap='*Methane emissions measured at the 30 sample sites within Cave Run Lake. The height of each "lollipop" corresponds to the emission rate shown on the vertical z-axis in units of milligrams CH~4~ per square meter of lake surface per hour.*' }


caveRunEqArea <- readOGR(dsn = paste(globalWd, "caveRun", sep=""), # get polygon
                           layer = "caveRunEqArea",  # shapefile name
                            verbose = FALSE)  #keep from reporting progress

# Dissolve strata from polygon
caveRunEqArea <- gUnaryUnion(caveRunEqArea, id = caveRunEqArea@data$Lake_Name)

caveRunSitesPlot@data <- merge(caveRunSitesPlot@data, # add emission to point shp
...
```{r echo=FALSE, warning=F, message=F, error=F, fig.width=7, fig.height=7, fig.cap='*Chlorophyll a concentrations measured at the 30 sample sites within Cave Run Lake. The height of each "lollipop" corresponds to the concentration shown on the vertical z-axis in units of micrograms per liter of water.*' }
##########################################################################
### chlorophyll plot
with(caveRunSitesPlot@data, {
...
plotColor <- ifelse(meanVariance.c.lake.lu$Lake_Name == "Cave Run Lake",
                    "red", "black")
