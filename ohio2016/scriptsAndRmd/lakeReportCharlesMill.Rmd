---
title: "Water Quality and Methane Emissions at Charles Mill Lake"
...
This preliminary report presents results from the USEPA 2016 measurement campaign relevant to Charles Mill Lake. These data will be included in a formal peer-reviewed publication to be submitted for publication in early 2018.  This preliminary report includes:  
...
ggmap(charlesMillSat) +
  ...
  geom_text(data=filter(charlesMillSites84@data, EvalStatus == "sampled"), #only main sites
...
```{r echo=FALSE, warning=F, message=F, error=F, fig.width=7, fig.height=7, fig.cap='*Methane emissions measured at the 15 sample sites within Charles Mill Lake. The height of each "lollipop" corresponds to the emission rate shown on the vertical z-axis in units of milligrams CH~4~ per square meter of lake surface per hour.*' }

# Get polygon
charlesMillEqArea <- readOGR(dsn = paste(globalWd, "charlesMill", sep=""), 
                           layer = "charlesMillEqArea",  # shapefile name
                            verbose = FALSE)  #keep from reporting progress

# Dissolve strata from polygon
charlesMillEqArea <- gUnaryUnion(charlesMillEqArea, id = charlesMillEqArea@data$Lake_Name)

# Add emission to point shp
charlesMillSitesPlot@data <- merge(charlesMillSitesPlot@data, 
...
coords = fortify(charlesMillEqArea)
...
with(charlesMillSitesPlot@data, {
...
```{r echo=FALSE, warning=F, message=F, error=F, fig.width=7, fig.height=7, fig.cap='*Chlorophyll a concentrations measured at the 15 sample sites within Charles Mill Lake. The height of each "lollipop" corresponds to the concentration shown on the vertical z-axis in units of micrograms per liter of water.*' }
##########################################################################
### chlorophyll plot
with(charlesMillSitesPlot@data, {
