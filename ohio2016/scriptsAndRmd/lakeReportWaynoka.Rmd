---
title: "Water Quality and Methane Emissions at Lake Waynoka"
...
This preliminary report presents results from the USEPA 2016 measurement campaign relevant to Lake Waynoka. These data will be included in a formal peer-reviewed publication to be submitted for publication in early 2018.  This preliminary report includes:  
...
3. A 3D map of the reservoir showing the measurement results for :
    + CH~4~ emissions
    + Chlorophyll a
4. Figures showing how Lake Waynoka compared to the other 31 reservoirs in the study in terms of:
    + CH~4~ emissions
    + Total phosphorus 
    + Total nitrogen
    + Chlorophyll a
5. Tables summarizing the other measured water quality values at each site within Lake Waynoka    
 
Thank you for your help in including Lake Waynoka in this project. 
...
ggmap(waynokaSat) +
  ylab("Latitude") +
  xlab ("Longitude") +
  geom_point(data=filter(waynokaSites84@data, EvalStatus == "sampled"), #only main sites
             aes(x=long, y=lat),
  ...
  geom_text(data=filter(waynokaSites84@data, EvalStatus == "sampled"), #only main sites
            aes(label=siteID, x=long, y=lat),
            hjust=1.2, vjust=0, size=3, color = "#00BFC4") +      #alternate color: "#F8766D"
  coord_equal() +
  ggtitle("Lake Waynoka Sample Sites")
```
...
Previous studies have found a general pattern of higher CH~4~ emissions in the river-reservoir transition (tributary) area, and lower emissions in the deeper downstream waters. This pattern was not observed at Lake Waynoka; with the exception of one "hot spot" on the southwest portion of the reservoir, similar CH~4~ emissions were measured across the reservoir. 

```{r echo=FALSE, warning=F, message=F, error=F, fig.width=7, fig.height=7, fig.cap='*Methane emissions measured at the 15 sample sites within Lake Waynoka. The height of each "lollipop" corresponds to the emission rate shown on the vertical z-axis in units of milligrams CH~4~ per square meter of lake surface per hour.*' }


waynokaEqArea <- readOGR(dsn = paste(globalWd, "waynoka", sep=""), # get polygon
                           layer = "waynokaEqArea",  # shapefile name
                            verbose = FALSE)  #keep from reporting progress

waynokaSitesPlot@data <- merge(waynokaSitesPlot@data, # add emission to point shp
                             filter(eqAreaData, Lake_Name == "Lake Waynoka") %>%
...
However, chlorophyll a concentrations in Lake Waynoka did show a clear gradient from upstream (NE) to downstream (SW) sites.  

```{r echo=FALSE, warning=F, message=F, error=F, fig.width=7, fig.height=7, fig.cap='*Chlorophyll a concentrations measured at the 15 sample sites within Lake Waynoka. The height of each "lollipop" corresponds to the concentration shown on the vertical z-axis in units of micrograms per liter of water.*' }
##########################################################################
### chlorophyll plot
with(waynokaSitesPlot@data, {
...
# Highlight Lake Waynoka (lake of interest) data with color
plotColor <- ifelse(meanVariance.c.lake.lu$Lake_Name == "Lake Waynoka",
                    "red", "black")
...
```{r warning=FALSE, echo=FALSE} 
waynokaTableChem <- filter(eqAreaData, Lake_Name == "Lake Waynoka",
                      EvalStatus == "sampled")  %>%
                  filter(!is.na(TN)) %>%
                  select(siteID, LatSamp, LongSmp, wtrDpth,  #site identification  
                          TNH4, TNO2.3, TN, TRP, TP)  #nutrient data 
                
#order by water depth at site 
waynokaTableChemOrder <- plyr::arrange(waynokaTableChem, wtrDpth)

kable(waynokaTableChemOrder,
      align = "c",
