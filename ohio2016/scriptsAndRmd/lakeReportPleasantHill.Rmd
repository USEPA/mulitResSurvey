---
title: "Water Quality and Methane Emissions at Pleasant Hill Lake"
...
This preliminary report presents results from the USEPA 2016 measurement campaign relevant to Pleasant Hill Lake. These data will be included in a formal peer-reviewed publication to be submitted for publication in early 2018.  This preliminary report includes:  
...
ggmap(pleasantHillSat) +
  ...
  geom_text(data=filter(pleasantHillSites84@data, EvalStatus == "sampled"), #only main sites
...
# Get polygon
pleasantHillEqArea <- readOGR(dsn = paste(globalWd, "pleasantHill", sep=""), 
                           layer = "pleasantHillEqArea",  # shapefile name
                            verbose = FALSE)  #keep from reporting progress

# Dissolve strata from polygon
pleasantHillEqArea <- gUnaryUnion(pleasantHillEqArea, id = pleasantHillEqArea@data$Lake_Name)

# Add emission to point shp
pleasantHillSitesPlot@data <- merge(pleasantHillSitesPlot@data, 
...
coords = fortify(pleasantHillEqArea)
...
with(pleasantHillSitesPlot@data, {
...
```{r echo=FALSE, warning=F, message=F, error=F, fig.width=7, fig.height=7, fig.cap='*Chlorophyll a concentrations measured at the 15 sample sites within Pleasant Hill Lake. The height of each "lollipop" corresponds to the concentration shown on the vertical z-axis in units of micrograms per liter of water.*' }
##########################################################################
### chlorophyll plot
with(pleasantHillSitesPlot@data, {
