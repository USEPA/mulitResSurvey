---
title: "Results from Acton Lake Survey Relevant to EC Study Design"
...
with(actonSitesPlot@data, {
  ch4_plot <- scatterplot3d_edit(x=coords$long, y=coords$lat, z=rep(0,length(coords$long)),   # x y and z data, respectively
                                 ...
                                 #cex.lab = 2,
                                   mar = c(5,6,4,2)+0.1,
                                   zlab=expression(mg~ CH[4]~ m^{-2}~ hr^{-1}),                      # z-axis label
  ...
  ch4_plot$points3d(x=xcoord, y=ycoord, z=as.numeric(as.character(ch4.drate.mg.h.best)), # x y and z data
                      ...
                      cex=(ch4.drate.mg.h.best/2) + 0.5          # scaling of point sizes - this will need to be adjusted for each variable
  )
})

ggsave('ohio2016/output/figures/ch4.diffActon.tiff',  # export as .tif
...
with(actonSitesPlot@data, {
  co2_plot <- scatterplot3d_edit(x=coords$long, y=coords$lat, z=rep(0,length(coords$long)),   # x y and z data, respectively
                                 ...
                                 #cex.lab = 2,
                                   mar = c(5,6,4,2)+0.1,
                                   zlab=expression(mg~ CO[2]~ m^{-2}~ hr^{-1}),                      # z-axis label
                                   zlim=range(0,60),                            # range of z data shown - I grabbed the max from previously plotting the methane data here. Set so when I add the points, the z-axis fits that data. 
                                   lty.axis=1,                                    # the line type of the axes
                                   axis=TRUE,                                     # wheather to include axes
                                   tick.marks = TRUE,
                                  label.tick.marks = TRUE,   #whether or not to include tick mark labels for all three axes
  ...
  # add the lollipop points
  co2_plot$points3d(x=xcoord, y=ycoord, z=as.numeric(as.character(-co2.drate.mg.h.best)), # x y and z data
                      col="#282830",                      # color of lines connecting points to lake outline
                      lwd=1.5,                            # line width of lines connecting points to lake outline
                      pch=21,                             # type of point to plot
                      bg="#FF0000",                       # fill color of points
                      type="h",                           # plot type = lines to the horizontal plane
                      cex=((-co2.drate.mg.h.best)/50) + 0.5          # scaling of point sizes - this will need to be adjusted for each variable
  )
})

ggsave('ohio2016/output/figures/co2.diffActon.tiff',  # export as .tif
       units="in",  # specify units for dimensions
       width=5,   # 1 column
       height=5, # Whatever works
       dpi=600,   # ES&T. 300-600 at PLOS One,
...
with(actonSitesPlot@data, {
  ...
  ch4_plot$points3d(x=xcoord, y=ycoord, z=as.numeric(as.character(ch4.erate.mg.h)), # x y and z data
                      ...
                      bg="#FF0000",                       # fill color of points
                      type="h",                           # plot type = lines to the horizontal plane
                      cex=(ch4.erate.mg.h/2) + 0.5          # scaling of point sizes - this will need to be adjusted for each variable
  )
})
```
...
with(actonSitesPlot@data, {
  co2_plot <- scatterplot3d_edit(x=coords$long, y=coords$lat, z=rep(0,length(coords$long)),   # x y and z data, respectively
                                 ...
                                 #cex.lab = 2,
                                   mar = c(5,6,4,2)+0.1,
                                   zlab=expression(mg~ CO[2]~ m^{-2}~ hr^{-1}),                      # z-axis label
                                   zlim=range(0,0.2),                            # range of z data shown - I grabbed the max from previously plotting the methane data here. Set so when I add the points, the z-axis fits that data. 
                                   lty.axis=1,                                    # the line type of the axes
                                   axis=TRUE,                                     # wheather to include axes
                                   tick.marks = TRUE,
                                  label.tick.marks = TRUE,   #whether or not to include tick mark labels for all three axes
  ...
  co2_plot$points3d(x=xcoord, y=ycoord, z=as.numeric(as.character(co2.erate.mg.h)), # x y and z data
                      ...
                      bg="#FF0000",                       # fill color of points
                      type="h",                           # plot type = lines to the horizontal plane
                      cex=(co2.erate.mg.h*10) + 0.5          # scaling of point sizes - this will need to be adjusted for each variable
  )
})
...
### Ebullitive co2 emission rate
with(actonSitesPlot@data, {
  vol_plot <- scatterplot3d_edit(x=coords$long, y=coords$lat, z=rep(0,length(coords$long)),   # x y and z data, respectively
                                   color="#282830",                               # color of lake outline
                                   lwd=2.0,                                       # line width of lake outline
                                   type="l",                                      # plot type - "l" = line, "h" = the lollipop style with a line  connecting to the horizontal plane 
                                   angle=80,                                      # angle aspect of plot (-40)
                                 ...
                                 #cex.lab = 2,
                                   mar = c(5,6,4,2)+0.1,
                                   zlab=expression(mL~gas~ m^{-2}~ hr^{-1}),                      # z-axis label
                                   zlim=range(0,100),                            # range of z data shown - I grabbed the max from previously plotting the methane data here. Set so when I add the points, the z-axis fits that data. 
                                   lty.axis=1,                                    # the line type of the axes
                                   axis=TRUE,                                     # wheather to include axes
                                   tick.marks = TRUE,
                                  label.tick.marks = TRUE,   #whether or not to include tick mark labels for all three axes
  ...
  # add the lollipop points
  vol_plot$points3d(x=xcoord, y=ycoord, z=as.numeric(as.character(ebMlHrM2)), # x y and z data
                      col="#282830",                      # color of lines connecting points to lake outline
                      lwd=1.5,                            # line width of lines connecting points to lake outline
                      pch=21,                             # type of point to plot
                      bg="#FF0000",                       # fill color of points
                      type="h",                           # plot type = lines to the horizontal plane
                      cex=(ebMlHrM2/50) + 0.5          # scaling of point sizes - this will need to be adjusted for each variable
  )
})
