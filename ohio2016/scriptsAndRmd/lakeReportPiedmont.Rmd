---
title: "Water Quality and Methane Emissions at Piedmont Lake, Ohio"
...
This preliminary report presents results from the USEPA 2016 measurement campaign relevant to Piedmont Lake, a reservoir located in east-central Ohio and managed by the Muskingum Watershed Conservancy District. These data will be included in a formal peer-reviewed publication to be submitted for publication in early 2018.  This preliminary report includes:  
...
3. A 3D map of the reservoir showing the measurement results for :
    + CH~4~ emissions
    + Chlorophyll a
4. Figures showing how Piedmont Lake compared to the other 31 reservoirs in the study in terms of:
    + CH~4~ emissions
    + Total phosphorus 
    + Total nitrogen
    + Chlorophyll a
5. Tables summarizing the other measured water quality values at each site within Piedmont Lake   
 
Thank you for your help in including Piedmont Lake in this project. 
...
Piedmont Lake, located in Belmont and Harrison counties in east central Ohio, is one of 16 reservoirs in the Muskingum Watershed Conservancy District. We sampled Piedmont Lake on June 27^th^ - 28^th^ of 2016. The sampling sites were chosen using a generalized random tessellation stratified design ("GRTS"), an approach which combines elements of systematic and random survey designs which allows for the random allocation of sampling sites with maximum  spatial coverage of the reservoir. We used a GPS and geographic information system (GIS) software to locate each sampling site (+/- 30 meters).  Chlorophyll a, dissolved oxygen, pH, specific conductivity, water temperature, turbidity, and CH~4~ emissions were measured at all sampling sites.  Nutrient concentrations were measured at one deep site near the dam and one shallow site near the inflow. 

```{r results='hide', message=FALSE, warning=FALSE, echo=FALSE, fig.width=7, fig.height=6.0, fig.cap='*Location of the fifteen sample sites within Piedmont Lake, located in east central Ohio. Water samples for nutrient analysis were collected from one deep and one shallow site. Satellite image from Google Maps.*'}


# Read and project spatial points dataframe for plotting
piedmontSitesPlot <- readOGR(dsn = paste(globalWd, "piedmont", sep=""), 
                          layer = "piedmontSitesEqAreaData")  # shapefile with data

piedmontSites84 <- spTransform(x = piedmontSitesPlot, #reproject
...
piedmontSites84@data <- mutate(piedmontSites84@data,
                              chem = ifelse(siteID %in% c("SU-33", "SU-10"), 
                                  "chem",
                                  "no_chem"))
...
ggmap(piedmontSat) +
  ylab("Latitude") +
  xlab ("Longitude") +
  geom_point(data=filter(piedmontSites84@data, EvalStatus == "sampled"), #only main sites
  ...
  geom_text_repel(data=filter(piedmontSites84@data, EvalStatus == "sampled"), #only main sites
 ...
 scale_color_manual(values = c("yellow", "white"), #alternate color: "#F8766D" "#00BFC4"
  ...
  guides(color = guide_legend(title = NULL)) +
   coord_equal() +
  ggtitle("Piedmont Lake Sample Sites")
```
...
Previous studies have found a general pattern of higher CH~4~ emissions in the river-reservoir transition (tributary) area, and lower emissions in the deeper downstream waters. This pattern was observed at Piedmont Lake, with higher emissions near the inflow in the southeastern tributary region and lower emissions in the deeper waters to the northwest, near the dam. Overall, Piedmont Lake had low CH~4~ emissions compared to other reservoirs in this survey (see Figure 4).

```{r echo=FALSE, warning=F, message=F, error=F, fig.width=7, fig.height=7, fig.cap='*Methane emissions measured at the 15 sample sites within Piedmont Lake, Ohio. The height of each "lollipop" corresponds to the emission rate shown on the vertical z-axis in units of milligrams CH~4~ per square meter of lake surface per hour.*' }

# Get polygon
piedmontEqArea84 <- readOGR(dsn = paste(globalWd, "piedmont", sep=""), 
                           layer = "piedmontEqArea84",  # shapefile name
                            verbose = FALSE)  #keep from reporting progress

# Dissolve strata from polygon
piedmontEqArea84 <- gUnaryUnion(piedmontEqArea84, id = piedmontEqArea84@data$Lake_Name)

# Add emission to point shp
piedmontSites84@data <- merge(piedmontSites84@data, 
                             filter(eqAreaData, Lake_Name == "Piedmont Lake") %>%
...
coords = fortify(piedmontEqArea84)
...
with(piedmontSites84@data, {
  ch4_plot <- scatterplot3d_edit(x=coords$long, y=coords$lat, z=rep(0,length(coords$long)),   # x y and z data, respectively
                                   ...
                                   angle=70,                                      # angle aspect of plot (-40)
  ...
  x <- dims[2] - 0.025*diff(dims[1:2])       # define x position of label
  y <- dims[3] + 0.13*diff(dims[3:4])       # define y position of label
  text(x,y,"Longitude", srt=75)             # add label.  srt sets angle
  
  # add dam label
  dims <- par("usr")                        # format of 'dims' is vector of: [xmin,xmax,ymin,ymax]
  x.d <- dims[1] + 0.17*diff(dims[1:2])       # define x position of label
  y.d <- dims[3] + 0.40*diff(dims[3:4])        # define y position of label
  text(x.d, y.d, "Dam")             # add label.  srt sets angle
  
  # add inflow label
  dims <- par("usr")                        # format of 'dims' is vector of: [xmin,xmax,ymin,ymax]
  x.i <- dims[1] + 0.66*diff(dims[1:2])       # define x position of label
  y.i <- dims[3] + 0.07*diff(dims[3:4])        # define y position of label
...
The chlorophyll a levels in Piedmont Lake displayed a similar pattern to that of the CH~4~ emissions, with higher levels in the inflow area and lower levels near the dam. 

```{r echo=FALSE, warning=F, message=F, error=F, fig.width=7, fig.height=7, fig.cap='*Chlorophyll a concentrations measured at the 15 sample sites within Piedmont Lake, Ohio. The height of each "lollipop" corresponds to the concentration shown on the vertical z-axis in units of micrograms per liter of water.*' }
##########################################################################
### chlorophyll plot
with(piedmontSites84@data, {
  chl_plot <- scatterplot3d_edit(x=coords$long, y=coords$lat, z=rep(0,length(coords$long)),   # x y and z data, respectively
                                   ...
                                   zlim=range(0, 15),                            # range of z data shown
                                   labs.line.z=-0.5,                              # adjustment of the z axis tick labels in margin lines
...
CH~4~ emission rate and chlorophyll a concentration were measured at all sampling sites.  We used the measurements to calculate mean values and an uncertainty range for the reservoir. The uncertainty measure we used is the 95% confidence interval (CI), which is similar to two standard deviations (sd) from the mean in a normally distributed data set (where 2*sd = 95.45%). In the case of this study, the majority of the uncertainty is due to the spatial variability of a given parameter within the reservoir, rather than uncertainty due to analytical errors. 
...
# Highlight Piedmont Lake (lake of interest) data with color
plotColor <- ifelse(meanVariance.c.lake.lu$Lake_Name == "Piedmont Lake",
                    "red", "black")
...
ggplot(meanVariance.c.lake.lu,
  ...
  theme(axis.title.y = element_blank(), # Eliminate x-axis title
    ...
    theme(axis.text.y = element_text(color = c(rep("black", 0), "red", rep("black", 31)),
        face = c(rep("plain", 0), "bold", rep("plain", 31)))) +
...
```{r echo=FALSE, warning=FALSE, fig.width=7, fig.height=7, fig.cap='*Mean and 95% confidence interval (CI) of the chlorophyll a concentration for each reservoir in this study, calculated from the values measured at >=15 sites within each reservoir.*'  }
...
ggplot(meanVariance.c.lake.lu,
  ...
  ggtitle("Mean Chlorophyll a (95% CI)") +
      theme(axis.text.y = element_text(color = c(rep("black", 3), "red", rep("black", 28)),
        face = c(rep("plain", 3), "bold", rep("plain", 28))))
```

\newpage

```{r echo=FALSE, warning=FALSE, fig.width=7, fig.height=7, fig.cap='*TP concentrations at a deep and shallow site in each reservoir included in the study.  Piedmont Lake is highlighted in red.  See Fig. 1 for sample site locations.*'  }
...
# Highlight Piedmont Lake (lake of interest) data with color
plotColor <- ifelse(tp.tn$fLake_Name == "Piedmont Lake",
                    "red", "black")
...
ggplot(tp.tn,
  ...
  theme(axis.text.y = element_text(color = c(rep("black", 5), "red", rep("black", 26)),
...
```{r echo=FALSE, warning=FALSE, fig.width=7, fig.height=7, fig.cap='*TN concentrations at a deep and shallow site in each reservoir included in the study.  Piedmont Lake is highlighted in red.  See Fig. 1 for sample site locations.*'  }
...
ggplot(tp.tn,
  ...
  theme(axis.text.y = element_text(color = c(rep("black", 4), "red", rep("black", 27)),
        face = c(rep("plain", 4), "bold", rep("plain", 27)))) +
  scale_color_manual(values = c("black", "red")) +
  guides(color=FALSE) +
  theme(legend.title = element_blank())
...
```{r warning=FALSE, echo=FALSE} 
piedmontTableChem <- filter(eqAreaData, Lake_Name == "Piedmont Lake",
                      EvalStatus == "sampled")  %>%
                  filter(!is.na(TN)) %>%
                  select(siteID, LatSamp, LongSmp, wtrDpth,  #site identification  
...
piedmontTableChemOrder <- plyr::arrange(piedmontTableChem, wtrDpth)
   
                         
kable(piedmontTableChemOrder,
      align = "c",
