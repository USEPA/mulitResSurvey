---
title: "Water Quality and Methane Emissions at Lake Mohawk"
...
This preliminary report presents results from the USEPA 2016 measurement campaign relevant to Lake Mohawk. These data will be included in a formal peer-reviewed publication to be submitted for publication in early 2018.  This preliminary report includes:  
...
3. A 3D map of the reservoir showing the measurement results for :
    + CH~4~ emissions
    + Chlorophyll a
4. Figures showing how Lake Mohawk compared to the other 31 reservoirs in the study in terms of:
    + CH~4~ emissions
    + Total phosphorus 
    + Total nitrogen
    + Chlorophyll a
5. Tables summarizing the other measured water quality values at each site within Lake Mohawk    
 
Thank you for your help in including Lake Mohawk in this project. 
...
We sampled Lake Mohawk on August 29^th^ - 30^th^ of 2016. The sampling sites were chosen using a generalized random tessellation stratified design ("GRTS"), an approach which combines elements of systematic and random survey designs which allows for the random allocation of sampling sites with maximum  spatial coverage of the reservoir. We used a GPS and geographic information system (GIS) software to locate each sampling site (+/- 30 meters).  
  
  

```{r results='hide', message=FALSE, warning=FALSE, echo=FALSE, fig.width=7, fig.height=7, fig.cap='*Location of the fifteen sample sites within Lake Mohawk. Satellite image from Google Maps.*'}


# Read and project spatial points dataframe for plotting
mohawkSitesPlot <- readOGR(dsn = paste(globalWd, "mohawk", sep=""), 
                          layer = "mohawkSitesEqAreaData")  # shapefile with data

mohawkSites84 <- spTransform(x = mohawkSitesPlot, #reproject
...
ggmap(mohawkSat) +
  ylab("Latitude") +
  xlab ("Longitude") +
  geom_point(data=filter(mohawkSites84@data, EvalStatus == "sampled"), #only main sites
             aes(x=long, y=lat),
  ...
  geom_text(data=filter(mohawkSites84@data, EvalStatus == "sampled"), #only main sites
            aes(label=siteID, x=long, y=lat),
            hjust=1.2, vjust=0, size=3, color = "#00BFC4") +      #alternate color: "#F8766D"
  coord_equal() +
  ggtitle("Lake Mohawk Sample Sites")
```
...
Previous studies have found a general pattern of higher CH~4~ emissions in the river-reservoir transition (tributary) area, and lower emissions in the deeper downstream waters. The two highest rates of CH~4~ emissions were observed near the southern tributary, but overall there was not a strong spatial pattern. 

```{r echo=FALSE, warning=F, message=F, error=F, fig.width=6, fig.height=7, fig.cap='*Methane emissions measured at the 15 sample sites within Lake Mohawk. The height of each "lollipop" corresponds to the emission rate shown on the vertical z-axis in units of milligrams CH~4~ per square meter of lake surface per hour.*' }


mohawkEqArea <- readOGR(dsn = paste(globalWd, "mohawk", sep=""), # get polygon
                           layer = "mohawkEqArea",  # shapefile name
                            verbose = FALSE)  #keep from reporting progress

mohawkSitesPlot@data <- merge(mohawkSitesPlot@data, # add emission to point shp
                             filter(eqAreaData, Lake_Name == "Lake Mohawk") %>%
...
with(mohawkSitesPlot@data, {
  ch4_plot <- scatterplot3d_edit(x=coords$long, y=coords$lat, z=rep(0,length(coords$long)),   # x y and z data, respectively
                                 ...
                                 #cex.lab = 2,
                                   ...
                                   zlim=range(0,30),                            # range of z data shown - I grabbed the max from previously plotting the methane data here. Set so when I add the points, the z-axis fits that data. 
...
Measured chlorophyll a concentrations in Lake Mohawk display a spatial pattern with higher levels to the east and south.  

```{r echo=FALSE, warning=F, message=F, error=F, fig.width=6, fig.height=7, fig.cap='*Chlorophyll a concentrations measured at the 15 sample sites within Lake Mohawk. The height of each "lollipop" corresponds to the concentration shown on the vertical z-axis in units of micrograms per liter of water.*' }
##########################################################################
### chlorophyll plot
with(mohawkSitesPlot@data, {
...
# Highlight Lake Mohawk (lake of interest) data with color
plotColor <- ifelse(meanVariance.c.lake.lu$Lake_Name == "Lake Mohawk",
                    "red", "black")
...
```{r warning=FALSE, echo=FALSE} 
mohawkTableChem <- filter(eqAreaData, Lake_Name == "Lake Mohawk",
                      EvalStatus == "sampled")  %>%
                  filter(!is.na(TN)) %>%
                  select(siteID, LatSamp, LongSmp, wtrDpth,  #site identification  
...
mohawkTableChemOrder <-plyr::arrange(mohawkTableChem, wtrDpth)   
                         
kable(mohawkTableChemOrder,
      align = "c",
