---
title: "Water Quality and Methane Emissions at Atwood Lake, Ohio"
...
This preliminary report presents results from the USEPA 2016 measurement campaign relevant to Atwood Lake, a reservoir located in east-central Ohio and managed by the Muskingum Watershed Conservancy District. These data will be included in a formal peer-reviewed publication to be submitted for publication in early 2018.  This preliminary report includes:  
...
atwoodSites84@data <- mutate(atwoodSites84@data,
                              chem = ifelse(siteID %in% c("SU-03", "SU-33"), 
                                  "chem",
                                  "no_chem"))
...
ggmap(atwoodSat) +
  ...
  geom_text_repel(data=filter(atwoodSites84@data, EvalStatus == "sampled"), #only main sites
...
```{r echo=FALSE, warning=F, message=F, error=F, fig.width=7, fig.height=7, fig.cap='*Methane emissions measured at the 15 sample sites within Atwood Lake, Ohio. The height of each "lollipop" corresponds to the emission rate shown on the vertical z-axis in units of milligrams CH~4~ per square meter of lake surface per hour.*' }

# Get polygon
atwoodEqArea84 <- readOGR(dsn = paste(globalWd, "atwood", sep=""), 
                           layer = "atwoodEqArea84",  # shapefile name
                            verbose = FALSE)  #keep from reporting progress

# Dissolve strata from polygon
atwoodEqArea84 <- gUnaryUnion(atwoodEqArea84, id = atwoodEqArea84@data$Lake_Name)

# Add emission to point shp
atwoodSites84@data <- merge(atwoodSites84@data, 
...
with(atwoodSites84@data, {
  ...
  # add inflow label
  dims <- par("usr")                        # format of 'dims' is vector of: [xmin,xmax,ymin,ymax]
  x.i <- dims[1] + 0.80*diff(dims[1:2])       # define x position of label
  y.i <- dims[3] + 0.22*diff(dims[3:4])        # define y position of label
...
The chlorophyll a levels in Atwood Lake displayed a clear gradient, decreasing at a consistent rate from the eastern inflow area to the western/downstream extent. 

```{r echo=FALSE, warning=F, message=F, error=F, fig.width=7, fig.height=7, fig.cap='*Chlorophyll a concentrations measured at the 15 sample sites within Atwood Lake, Ohio. The height of each "lollipop" corresponds to the concentration shown on the vertical z-axis in units of micrograms per liter of water.*' }
##########################################################################
### chlorophyll plot
with(atwoodSites84@data, {
  ...
  # add inflow label
  dims <- par("usr")                        # format of 'dims' is vector of: [xmin,xmax,ymin,ymax]
  x.i <- dims[1] + 0.8*diff(dims[1:2])       # define x position of label
  y.i <- dims[3] + 0.22*diff(dims[3:4])        # define y position of label
  text(x.i, y.i, "Inflow")                       # add label. 
...
```{r echo=FALSE, warning=FALSE, fig.width=7, fig.height=7, fig.cap='*TP concentrations at a deep and shallow site in each reservoir included in the study.  Atwood Lake is highlighted in red.  See Fig. 1 for sample site locations.*'  }
...
# Highlight Atwood Lake (lake of interest) data with color
plotColor <- ifelse(tp.tn$fLake_Name == "Atwood Lake",
                    "red", "black")
...
ggplot(tp.tn,
  ...
  theme(axis.text.y = element_text(color = c(rep("black", 15), "red", rep("black", 16)),
        face = c(rep("plain", 15), "bold", rep("plain", 16)))) +
...
```{r echo=FALSE, warning=FALSE, fig.width=7, fig.height=7, fig.cap='*TN concentrations at a deep and shallow site in each reservoir included in the study.  Atwood Lake is highlighted in red.  See Fig. 1 for sample site locations.*'  }
