...
The US Environmental Protection Agency (USEPA) is conducting an investigation of methane (CH~4~) dynamics in reservoirs. Methane is a potent greenhouse gas that is produced by microorganisms in reservoir sediments. The USEPA is supporting research to estimate the magnitude of methane emissions from reservoirs in the United States. 

The USEPA measured CH~4~ emissions from 32 reservoirs in Ohio, Indiana, and Kentucky during the summer of 2016. We typically designated 15 sites in each lake, where we measured CH~4~ emissions and several water quality indicators. CH~4~ emissions were measured using a device which captures CH~4~-rich bubbles as they rise through the water column toward the atmosphere. A sonde was used to measure chlorophyll a, dissolved oxygen, pH, specific conductivity, water temperature, and turbidity just below the water surface at each of the 15 sites. Additionally, nutrient chemistry was analyzed at one shallow and one deep site for each reservoir.  
...
We sampled Acton Lake on May 31^st^ of 2016. The sites were designated via statistical software (the generalized random tesselation stratified design, or "GRTS"), an approach we chose because it reduced bias while maintaining spatial coverage. We used a GPS and geographic information system (GIS) software to ensure we were within 30 meters of each designated site.  
...
Previous studies have found a general pattern of higher CH~4~ emissions in the river-reservoir transition (tributary) area, and lower emissions in the deeper waters. Acton Lake did not match this pattern, perhaps because it is a relatively small, shallow water body.   
```{r echo=FALSE, warning=F, error=F, message=F, fig.width=7, fig.height=8, fig.cap='Methane (CH~4~) emissions at the 15 sample sites within Acton Lake. The height of the "lollipop" corresponds to the emission rate shown on the vertical z-axis in units of mg CH~4~ m^-2^ hr^-1^. The x- and y-axes are oriented east-west and north-south, respectively.' }
...
```{r echo=FALSE, warning=F, message=F, error=F, fig.width=7, fig.height=8}
##########################################################################
### chlorophyll plot
with(actonSitesPlot@data, {
...
We used the results from the 15 sites to calculate mean values and the 95% confidence interval (CI) for each measurement. The 95% CI is a measure of uncertainty and is a similar parameter as two standard deviations (sd) from the mean in a normally distributed data set (where 2*sd = 95.45%). In the case of this study, the majority of the uncertainty is due to the spatial variability of a given parameter across the reservoir, rather than uncertainty due to analytical errors.  



```{r echo=FALSE, warning=FALSE, fig.width=7, fig.height=7, fig.cap='Mean and 95% confidence interval of the CH[4] emission rate measured at >=15 sites in each reservoir.'  }
####################################################################################
...
```{r echo=FALSE, warning=FALSE, fig.width=7, fig.height=7, fig.cap='Mean and 95% confidence interval of the CH[4] emission rate measured at >=15 sites in each reservoir.'  }
# DO -- commented out because currently the grts calculation does not process the mean and 95% CI for DO
