---
title: "Water Quality and Methane Emissions at Apple Valley Lake"
...
This preliminary report presents results from the USEPA 2016 measurement campaign relevant to Apple Valley Lake. These data will be included in a formal peer-reviewed publication to be submitted for publication in early 2018.  This preliminary report includes:  
...
3. A 3D map of the reservoir showing the measurement results for :
    + CH~4~ emissions
    + Chlorophyll a
4. Figures showing how Apple Valley Lake compared to the other 31 reservoirs in the study in terms of:
    + CH~4~ emissions
    + Total phosphorus 
    + Total nitrogen
    + Chlorophyll a
5. Tables summarizing the other measured water quality values at each site within Apple Valley Lake    
 
Thank you for your help in including Apple Valley Lake in this project. 
...
We sampled Apple Valley Lake on July 20^th^ - 21^st^ of 2016. The sampling sites were chosen using a generalized random tessellation stratified design ("GRTS"), an approach which combines elements of systematic and random survey designs which allows for the random allocation of sampling sites with maximum  spatial coverage of the reservoir. We used a GPS and geographic information system (GIS) software to locate each sampling site (+/- 30 meters).  
  
  

```{r results='hide', message=FALSE, warning=FALSE, echo=FALSE, fig.width=7, fig.height=7, fig.cap='*Location of the fifteen sample sites within Apple Valley Lake. Satellite image from Google Maps.*'}


# Read and project spatial points dataframe for plotting
appleValleySitesPlot <- readOGR(dsn = paste(globalWd, "appleValley", sep=""), 
                          layer = "appleValleySitesEqAreaData")  # shapefile with data

appleValleySites84 <- spTransform(x = appleValleySitesPlot, #reproject
...
ggmap(appleValleySat) +
  ylab("Latitude") +
  xlab ("Longitude") +
  geom_point(data=filter(appleValleySites84@data, EvalStatus == "sampled"), #only main sites
             aes(x=long, y=lat),
  ...
  geom_text(data=filter(appleValleySites84@data, EvalStatus == "sampled"), #only main sites
            aes(label=siteID, x=long, y=lat),
            hjust=1.2, vjust=0, size=3, color = "#00BFC4") +      #alternate color: "#F8766D"
  coord_equal() +
  ggtitle("Apple Valley Lake Sample Sites")
```
...
Previous studies have found a general pattern of higher CH~4~ emissions in the river-reservoir transition (tributary) area, and lower emissions in the deeper downstream waters. The observations at Apple Valley Lake followed this pattern. 

```{r echo=FALSE, warning=F, message=F, error=F, fig.width=7, fig.height=7, fig.cap='*Methane emissions measured at the 15 sample sites within Apple Valley Lake. The height of each "lollipop" corresponds to the emission rate shown on the vertical z-axis in units of milligrams CH~4~ per square meter of lake surface per hour.*' }


appleValleyEqArea <- readOGR(dsn = paste(globalWd, "appleValley", sep=""), # get polygon
                           layer = "appleValleyEqArea",  # shapefile name
                            verbose = FALSE)  #keep from reporting progress

appleValleySitesPlot@data <- merge(appleValleySitesPlot@data, # add emission to point shp
                             filter(eqAreaData, Lake_Name == "Apple Valley Lake") %>%
...
coords = fortify(appleValleyEqArea)
...
with(appleValleySitesPlot@data, {
  ch4_plot <- scatterplot3d_edit(x=coords$long, y=coords$lat, z=rep(0,length(coords$long)),   # x y and z data, respectively
                                 ...
                                 #cex.lab = 2,
                                   ...
                                   zlim=range(0,40),                            # range of z data shown - I grabbed the max from previously plotting the methane data here. Set so when I add the points, the z-axis fits that data. 
...
Similarly, chlorophyll a concentrations in Apple Valley Lake also had a clear gradient from upstream to downstream sites.  

```{r echo=FALSE, warning=F, message=F, error=F, fig.width=7, fig.height=7, fig.cap='*Chlorophyll a concentrations measured at the 15 sample sites within Apple Valley Lake. The height of each "lollipop" corresponds to the concentration shown on the vertical z-axis in units of micrograms per liter of water.*' }
##########################################################################
### chlorophyll plot
with(appleValleySitesPlot@data, {
...
# Highlight Apple Valley Lake (lake of interest) data with color
plotColor <- ifelse(meanVariance.c.lake.lu$Lake_Name == "Apple Valley Lake",
                    "red", "black")
...
```{r echo=FALSE, warning=FALSE, fig.width=7, fig.height=7, fig.cap='*Mean and 95% confidence interval (CI) of the total phosphorus (TP) concentration for each reservoir in this study, calculated from the values measured at >=15 sites within each reservoir.*'  }
...
```{r echo=FALSE, warning=FALSE, fig.width=7, fig.height=7, fig.cap='*Mean and 95% confidence interval (CI) of the total nitrogen (TN) concentration for each reservoir in this study, calculated from the values measured at >=15 sites within each reservoir.*'  }
...
```{r warning=FALSE, echo=FALSE} 
appleValleyTableChem <- filter(eqAreaData, Lake_Name == "Apple Valley Lake",
                      EvalStatus == "sampled")  %>%
                  filter(!is.na(TN)) %>%
                  select(siteID, LatSamp, LongSmp, wtrDpth,  #site identification  
...
appleValleyTableChemOrder <- plyr::arrange(
                             appleValleyTableChem, wtrDpth) 
   
                         
kable(appleValleyTableChemOrder,
      align = "c",
