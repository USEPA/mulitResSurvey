...
In the next phase of the research program, Dr. Beaulieu plans to use field surveys to identify the landscape and geomorphic factors that control CH4 emission rates from reservoirs within the state of Ohio.  Ohio was chosen based on proximity to the EPA’s research facility in Cincinnati and the east to west agriculture/forest land-use gradient in the state.  
...
```{r}
# SET WORKING DIRECTORY.  MUST CHANGE FOR EACH USER
# setwd("\\\\Aa.ad.epa.gov/ord/CIN/Users/main/F-K/JBEAULIE/Net MyDocuments/Documents/research/EPA/East Fork Reservoir/ACE/multiResSurvey/ohio")

ohioRes <- read_excel("inputData/ohioReservoirGeomorphology12.17.15.xlsx") 
...
```{r}
# Reservoir geometry
ldGeom <- read.xls("inputData/WA 3-93 reservoirsurfacegeometry.xlsx", sheet="Task 1a", stringsAsFactors = FALSE)
...
# Watershed land use
ldLu <- read.xls("inputData/WatershedAreaCharacterization.xlsx", sheet="FinalForm", stringsAsFactors = FALSE)
str(ldLu)
...
# Reservoir Hydrology
ldHyd <- read.xls("inputData/LouisDistReservoirHydrology.xlsx", sheet="hydrology", stringsAsFactors=FALSE)
