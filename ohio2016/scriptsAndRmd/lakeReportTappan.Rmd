---
title: "Water Quality and Methane Emissions at Tappan Lake, Ohio"
...
This preliminary report presents results from the USEPA 2016 measurement campaign relevant to Tappan Lake, a reservoir located in east-central Ohio and managed by the Muskingum Watershed Conservancy District. These data will be included in a formal peer-reviewed publication to be submitted for publication in early 2018.  This preliminary report includes:  
...
3. A 3D map of the reservoir showing the measurement results for :
    + CH~4~ emissions
    + Chlorophyll a
4. Figures showing how Tappan Lake compared to the other 31 reservoirs in the study in terms of:
    + CH~4~ emissions
    + Total phosphorus 
    + Total nitrogen
    + Chlorophyll a
5. Tables summarizing the other measured water quality values at each site within Tappan Lake   
 
Thank you for your help in including Tappan Lake in this project. 
...
tappanSites84@data <- mutate(tappanSites84@data,
                              chem = ifelse(siteID %in% c("SU-28", "SU-07"), 
                                  "chem",
                                  "no_chem"))
...
ggmap(tappanSat) +
  ylab("Latitude") +
  xlab ("Longitude") +
  geom_point(data=filter(tappanSites84@data, EvalStatus == "sampled"), #only main sites
  ...
  geom_text_repel(data=filter(tappanSites84@data, EvalStatus == "sampled"), #only main sites
 ...
 scale_color_manual(values = c("yellow", "white"), #alternate color: "#F8766D" "#00BFC4"
  ...
  guides(color = guide_legend(title = NULL)) +
   coord_equal() +
  ggtitle("Tappan Lake Sample Sites")
```
...
Previous studies have found a general pattern of higher CH~4~ emissions in the river-reservoir transition (tributary) area, and lower emissions in the deeper downstream waters. This pattern was observed at Tappan Lake, with higher emissions in the eastern tributary region and lower emissions in the deeper waters to the west. Overall, Tappan Lake had low CH~4~ emissions compared to other reservoirs in this survey (see Figure 4).

```{r echo=FALSE, warning=F, message=F, error=F, fig.width=7, fig.height=7, fig.cap='*Methane emissions measured at the 15 sample sites within Tappan Lake, Ohio. The height of each "lollipop" corresponds to the emission rate shown on the vertical z-axis in units of milligrams CH~4~ per square meter of lake surface per hour.*' }

# Get polygon
tappanEqArea84 <- readOGR(dsn = paste(globalWd, "tappan", sep=""), 
                           layer = "tappanEqArea84",  # shapefile name
                            verbose = FALSE)  #keep from reporting progress

# Dissolve strata from polygon
tappanEqArea84 <- gUnaryUnion(tappanEqArea84, id = tappanEqArea84@data$Lake_Name)

# Add emission to point shp
tappanSites84@data <- merge(tappanSites84@data, 
                             filter(eqAreaData, Lake_Name == "Tappan Lake") %>%
...
with(tappanSites84@data, {
  ...
  y.d <- dims[3] + 0.25*diff(dims[3:4])        # define y position of label
  text(x.d, y.d, "Dam")             # add label.  srt sets angle
  
  # add inflow label
  dims <- par("usr")                        # format of 'dims' is vector of: [xmin,xmax,ymin,ymax]
  x.i <- dims[1] + 0.75*diff(dims[1:2])       # define x position of label
...
The chlorophyll a levels in Tappan Lake displayed a clear gradient, decreasing at a consistent rate from the eastern inflow area to the western/downstream extent. 

```{r echo=FALSE, warning=F, message=F, error=F, fig.width=7, fig.height=7, fig.cap='*Chlorophyll a concentrations measured at the 15 sample sites within Tappan Lake, Ohio. The height of each "lollipop" corresponds to the concentration shown on the vertical z-axis in units of micrograms per liter of water.*' }
##########################################################################
### chlorophyll plot
with(tappanSites84@data, {
  ...
  y.d <- dims[3] + 0.13*diff(dims[3:4])        # define y position of label
  text(x.d, y.d, "Dam")             # add label.  srt sets angle
  
  # add inflow label
  dims <- par("usr")                        # format of 'dims' is vector of: [xmin,xmax,ymin,ymax]
  x.i <- dims[1] + 0.76*diff(dims[1:2])       # define x position of label
  y.i <- dims[3] + 0.17*diff(dims[3:4])        # define y position of label
  text(x.i, y.i, "Inflow")                       # add label. 
...
# Highlight Tappan Lake (lake of interest) data with color
plotColor <- ifelse(meanVariance.c.lake.lu$Lake_Name == "Tappan Lake",
                    "red", "black")
...
```{r echo=FALSE, warning=FALSE, fig.width=7, fig.height=7, fig.cap='*TP concentrations at a deep and shallow site in each reservoir included in the study.  Tappan Lake is highlighted in red.  See Fig. 1 for sample site locations.*'  }
...
# Highlight Charles Mill Lake (lake of interest) data with color
plotColor <- ifelse(tp.tn$fLake_Name == "Tappan Lake",
                    "red", "black")
...
ggplot(tp.tn,
  ...
  theme(axis.text.y = element_text(color = c(rep("black", 13), "red", rep("black", 18)),
        face = c(rep("plain", 13), "bold", rep("plain", 18)))) +
...
```{r echo=FALSE, warning=FALSE, fig.width=7, fig.height=7, fig.cap='*TN concentrations at a deep and shallow site in each reservoir included in the study.  Tappan Lake is highlighted in red.  See Fig. 1 for sample site locations.*'  }
...
ggplot(tp.tn,
  ...
  theme(axis.text.y = element_text(color = c(rep("black", 12), "red", rep("black", 19)),
        face = c(rep("plain", 12), "bold", rep("plain", 19)))) +
  scale_color_manual(values = c("black", "red")) +
  guides(color=FALSE) +
  theme(legend.title = element_blank())
...
```{r warning=FALSE, echo=FALSE} 
tappanTableChem <- filter(eqAreaData, Lake_Name == "Tappan Lake",
                      EvalStatus == "sampled")  %>%
                  filter(!is.na(TN)) %>%
                  select(siteID, LatSamp, LongSmp, wtrDpth,  #site identification  
...
tappanTableChemOrder <- plyr::arrange(tappanTableChem, wtrDpth)
   
                         
kable(tappanTableChemOrder,
      align = "c",
