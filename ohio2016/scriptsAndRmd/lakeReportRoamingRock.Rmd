---
title: "Water Quality and Methane Emissions at Lake Roaming Rock"
...
This preliminary report presents results from the USEPA 2016 measurement campaign relevant to Lake Roaming Rock. These data will be included in a formal peer-reviewed publication to be submitted for publication in early 2018.  This preliminary report includes:  
...
3. A 3D map of the reservoir showing the measurement results for :
    + CH~4~ emissions
    + Chlorophyll a
4. Figures showing how Lake Roaming Rock compared to the other 31 reservoirs in the study in terms of:
    + CH~4~ emissions
    + Total phosphorus 
    + Total nitrogen
    + Chlorophyll a
5. Tables summarizing the other measured water quality values at each site within Lake Roaming Rock    
 
Thank you for your help in including Lake Roaming Rock in this project. 
...
We sampled Lake Roaming Rock on September 8^th^ - 9^th^ of 2016. The sampling sites were chosen using a generalized random tessellation stratified design ("GRTS"), an approach which combines elements of systematic and random survey designs which allows for the random allocation of sampling sites with maximum  spatial coverage of the reservoir. We used a GPS and geographic information system (GIS) software to locate each sampling site (+/- 30 meters).  
  
  

```{r results='hide', message=FALSE, warning=FALSE, echo=FALSE, fig.width=7, fig.height=7, fig.cap='*Location of the fifteen sample sites within Lake Roaming Rock. Satellite image from Google Maps.*'}


# Read and project spatial points dataframe for plotting
roamingRockSitesPlot <- readOGR(dsn = paste(globalWd, "roamingRock", sep=""), 
                          layer = "roamingRockSitesEqAreaData")  # shapefile with data

roamingRockSites84 <- spTransform(x = roamingRockSitesPlot, #reproject
...
ggmap(roamingRockSat) +
  ylab("Latitude") +
  xlab ("Longitude") +
  geom_point(data=filter(roamingRockSites84@data, EvalStatus == "sampled"), #only main sites
             aes(x=long, y=lat),
  ...
  geom_text(data=filter(roamingRockSites84@data, EvalStatus == "sampled"), #only main sites
            aes(label=siteID, x=long, y=lat),
            hjust=1.2, vjust=0, size=3, color = "#00BFC4") +      #alternate color: "#F8766D"
  coord_equal() +
  ggtitle("Lake Roaming Rock Sample Sites")
```
...
Previous studies have found a general pattern of higher CH~4~ emissions in the river-reservoir transition (tributary) area, and lower emissions in the deeper downstream waters. Lake Roaming Rock displayed this pattern: the highest emissions were observed in the south tributary and near the northeast tributary inflow. 

```{r echo=FALSE, warning=F, message=F, error=F, fig.width=7, fig.height=7, fig.cap='*Methane emissions measured at the 15 sample sites within Lake Roaming Rock. The height of each "lollipop" corresponds to the emission rate shown on the vertical z-axis in units of milligrams CH~4~ per square meter of lake surface per hour.*' }


roamingRockEqArea <- readOGR(dsn = paste(globalWd, "roamingRock", sep=""), # get polygon
                           layer = "roamingRockEqArea",  # shapefile name
                            verbose = FALSE)  #keep from reporting progress

roamingRockSitesPlot@data <- merge(roamingRockSitesPlot@data, # add emission to point shp
                             filter(eqAreaData, Lake_Name == "Lake Roaming Rock") %>%
...
coords = fortify(roamingRockEqArea)
...
with(roamingRockSitesPlot@data, {
...
However, chlorophyll a concentrations in Lake Roaming Rock did not show a clear difference between upstream and downstream sites, and were relatively low overall (see Figure 5).  

```{r echo=FALSE, warning=F, message=F, error=F, fig.width=7, fig.height=7, fig.cap='*Chlorophyll a concentrations measured at the 15 sample sites within Lake Roaming Rock. The height of each "lollipop" corresponds to the concentration shown on the vertical z-axis in units of micrograms per liter of water.*' }
##########################################################################
### chlorophyll plot
with(roamingRockSitesPlot@data, {
...
# Highlight Lake Roaming Rock (lake of interest) data with color
plotColor <- ifelse(meanVariance.c.lake.lu$Lake_Name == "Lake Roaming Rock",
                    "red", "black")
...
```{r warning=FALSE, echo=FALSE} 
roamingRockTableChem <- filter(eqAreaData, Lake_Name == "Lake Roaming Rock",
                      EvalStatus == "sampled")  %>%
                  filter(!is.na(TN)) %>%
                  select(siteID, LatSamp, LongSmp, wtrDpth,  #site identification  
...
roamingRockTableChemOrder <- plyr::arrange(roamingRockTableChem, wtrDpth)
   
                         
kable(roamingRockTableChemOrder,
      align = "c",
