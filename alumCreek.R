...
### We decided to set the number of main sites in the tributary area to 7, since it is a relatively large area (1 sq km)
...
alumCreekSites <- grts(design=alumCreekDsgn,
                        ...
                        mdcaty="Sections",
                        shapefile=TRUE,
                        out.shape=paste(rootDir, "alumCreek/alumCreekSites", sep=""),
                        prjfilename=paste(rootDir, "alumCreek/alumCreekEqArea", sep=""))


# Print the initial six lines of the survey design
head(alumCreekSites@data)

# Print the survey design summary
summary(alumCreekSites)

plot(alumCreekEqArea)
points(alumCreekSites$xcoord, alumCreekSites$ycoord)
...
alumCreekSitesPlot <- readOGR(dsn = paste(rootDir, "alumCreek", sep=""), 
                               layer = "alumCreekSites")  # shapefile created with grts function
alumCreekSites84 <- spTransform(x = alumCreekSitesPlot, #reproject
...
ggmap(alumCreekSat) +
  ylab("Latitude") +
  xlab ("Longitude") +
  geom_polygon(data=alumCreekEqArea84.f, aes(long, lat, group=group, fill=Sections)) +
